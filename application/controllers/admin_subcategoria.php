...
class Admin_subcategoria extends CI_Controller {
	...
	public function combo(){
		...
		if(strlen($id)>0){
			$data=$this->subcategoriaModel->getAllByCategoria($id);
		}else{
			$data=$this->subcategoriaModel->getAll();
		}		
	...
	public function save(){
		...
		$data=array(
				"categoria_id"=>trim($this->input->get_post("categoria_id")),
		...
		if($flag==true){
			...
			if($id!=false && strlen($id)>0 && isset($id)==true){
				$num=intval($this->subcategoriaModel->update($id,$data));
			}else{
				$id=intval($this->subcategoriaModel->create($data));
			}
			if($num>0 || $id>0){
...
/* End of file admin_subcategoria.php */
/* Location: ./application/controllers/admin_subcategoria.php */